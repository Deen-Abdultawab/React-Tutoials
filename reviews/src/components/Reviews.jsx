...
export const Reviews = ({activeReview, getActiveReview, allReviews}) => {
    ...
    return (
        <article className='py-[24px] px-[36px] bg-[#FFFFFF] shadow-2xl w-[90%] max-w-[600px]'>
            <div className='w-[150px] h-[150px] rounded-[100%] mx-auto relative bg-[white] overflow-hidden'>
            <img src={activeReview.image} alt="" className='w-full h-full object-cover'/>
            </div>
            ...
            <button className='bg-[#C1BEFF] rounded-[4px] text-[#645CFF] text-[13px] tracking-[1px] py-[6px] px-[12px] mx-auto w-fit block capitalize' onClick={()=>getActiveReview(randomId)}>suprise me</button>
        </article>
    )
}